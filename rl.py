...
class DQNAgent:
    ...
    def build_model(self):
        ...
        model.add(layers.Dense(32, activation='relu', input_shape=self.state_shape))
...
for epoch in range(num_epochs):
    ...
    while not done:
        ...
        if rospy.Time.now() - start_time > timeout_duration:
            print("Timeout reached. Training terminated.")
            done = True
            break
        ...
        # Check for crash condition and break the loop if a crash occurs
        if check_crash_condition():
            print("Turtlebot crashed. Training terminated.")
            done = True
            print()
            break
    if epoch%10==0:
        # Save the trained model
        model_path = '/home/sri2/AIE20006/src/lab7_wanderer/src/modified/'+str(epoch)
